...
import random
from GameSet import screen, wall_image, box_image, medium_font, ticon,ticon2
    #, sh_sd
...
class Tank:
    ...
    def draw(self):
        ...
        if self.direction == Direction.RIGHT:
            pygame.draw.line(screen, self.color, tank_c, (tank_c[0] + self.width, tank_c[1]), 5)
        if self.direction == Direction.LEFT:
            pygame.draw.line(screen, self.color, tank_c, (tank_c[0] - self.width, tank_c[1]), 5)
        if self.direction == Direction.UP:
            pygame.draw.line(screen, self.color, tank_c, (tank_c[0], tank_c[1] - self.width), 5)
        if self.direction == Direction.DOWN:
            pygame.draw.line(screen, self.color, tank_c, (tank_c[0], tank_c[1] + self.width), 5)
